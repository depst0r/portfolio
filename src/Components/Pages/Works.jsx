    ...
    if (error) return <ErrorMessage/>
       ...
       return (
        ...
        className="project__cards">
                {repos.map(rep => (
                    <div className='project__card' key={rep.id}>   
                ...
                <div className="card__lang">
                    <span className="title-h6">{rep.language}</span>
                </div>
                <div className="card__info">
                    <div className="card__title">{rep.name}</div>
                    <div className="card__sub title-h6">{rep.license?.name ?? 'No license'}</div>
                    <div className="card__buttons">
                        <a href={rep.homepage} type='button' target='blank'>Live 🔗</a>
                        <a href={rep.html_url} type='button' target='blank'>Github</a>
                        </div>
                </div>
                </div> 
            ))}
